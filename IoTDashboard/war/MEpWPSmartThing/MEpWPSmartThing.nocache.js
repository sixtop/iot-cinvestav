...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "MEpWPSmartThing" because\n';
  ...
  $wnd.__gwt_activeModules['MEpWPSmartThing'] = {
    'moduleName' : 'MEpWPSmartThing',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/MEpWPSmartThing.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['MEpWPSmartThing'].superdevmode = true;

  var expectedSuffix = '/MEpWPSmartThing.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + MEpWPSmartThing + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:MEpWPSmartThing';
  var devModeSessionKey = '__gwtDevModeSession:MEpWPSmartThing';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/MEpWPSmartThing/MEpWPSmartThing.nocache.js';
  ...
  $wnd.__gwt_activeModules['MEpWPSmartThing'].canRedirect = true;
  $wnd.__gwt_activeModules['MEpWPSmartThing'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING MEpWPSmartThing';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/MEpWPSmartThing?user.agent=' + ua + '&_callback=' + callback;
