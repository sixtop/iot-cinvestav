...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "MEpSensor" because\n';
  ...
  $wnd.__gwt_activeModules['MEpSensor'] = {
    'moduleName' : 'MEpSensor',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/MEpSensor.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['MEpSensor'].superdevmode = true;

  var expectedSuffix = '/MEpSensor.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + MEpSensor + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:MEpSensor';
  var devModeSessionKey = '__gwtDevModeSession:MEpSensor';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/MEpSensor/MEpSensor.nocache.js';
  ...
  $wnd.__gwt_activeModules['MEpSensor'].canRedirect = true;
  $wnd.__gwt_activeModules['MEpSensor'].superdevmode = true;
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/MEpSensor?user.agent=' + ua + '&_callback=' + callback;
