...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "MEpWPController" because\n';
  ...
  $wnd.__gwt_activeModules['MEpWPController'] = {
    'moduleName' : 'MEpWPController',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/MEpWPController.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['MEpWPController'].superdevmode = true;

  var expectedSuffix = '/MEpWPController.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + MEpWPController + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:MEpWPController';
  var devModeSessionKey = '__gwtDevModeSession:MEpWPController';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/MEpWPController/MEpWPController.nocache.js';
  ...
  $wnd.__gwt_activeModules['MEpWPController'].canRedirect = true;
  $wnd.__gwt_activeModules['MEpWPController'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING MEpWPController';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/MEpWPController?user.agent=' + ua + '&_callback=' + callback;
