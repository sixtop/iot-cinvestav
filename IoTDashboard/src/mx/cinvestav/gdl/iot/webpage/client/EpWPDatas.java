...
public class EpWPDatas extends IoTEntryPoint {
	...
	////////////////////////////////////////////////////////////	
		 ...
		 * 
		 * @Override public void onClick(ClickEvent event) { if (chFilter.getValue()) {
		 * 
		 * for (int i = 0; i < arrayFilter.length; i++) { arrayFilter[i].setEnabled(true); } } else {
		 * 
		 * for (int i = 0; i < arrayFilter.length; i++) { arrayFilter[i].setEnabled(false); }
		 * 
		 * }
		 * 
		 * } });
		 * 
		 * decoratorPanel.add(tableFilter); formPanel.add(decoratorPanel);
		 */
	...
	public void fillExperiment1(){
		lbController.addChangeHandler(new ChangeHandler() {
			...
			public void onChange(ChangeEvent event) {
				...
				/*
				 * entityService.getProperties(new ControllerPropertyDTO(), idController, new
				 * AsyncCallback<List<ControllerPropertyDTO>>() {
				 * 
				 * @Override public void onFailure(Throwable caught) { dbWait.hide(); // TODO:
				 * Window.alert(caught.getMessage()); }
				 * 
				 * @Override public void onSuccess(List<ControllerPropertyDTO> resultP) { int c = 0;
				 * 
				 * for (int i = 0; i < resultP.size(); i++) { if (resultP.get(i).getName().startsWith("Filter_")) { c++;
				 * } }
				 * 
				 * arrayFilter = new CheckBox[c]; c=0;
				 * 
				 * filtersPanel.setSpacing(4); for (int i = 0; i < resultP.size(); i++) { if
				 * (resultP.get(i).getName().startsWith("Filter_")) { arrayFilter[c] = new
				 * CheckBox(resultP.get(i).getName().substring(7, resultP.get(i).getName().length()));
				 * arrayFilter[c].setName(resultP.get(i).getValue()); arrayFilter[c].setValue(true);
				 * arrayFilter[c].setEnabled(false); tableFilter.setWidget(i + 1, 1, arrayFilter[c]); c++; } }
				 * 
				 * } });
				 */
			...
			public void onChange(ChangeEvent event) {
				...
				entityService.getEntity(new SensorDTO(), null, new AsyncCallback<List<SensorDTO>>() {
					...
					public void onSuccess(List<SensorDTO> result) {
						...
						/*
						 * arrayTypeSensor=new CheckBox[lbTypeSensor.getItemCount()];
						 * 
						 * for (int i = 0; i < lbTypeSensor.getItemCount(); i++) { arrayTypeSensor[i] = new
						 * CheckBox(lbTypeSensor.getItemText(i)); arrayTypeSensor[i].setName(lbTypeSensor.getValue(i));
						 * arrayTypeSensor[i].setValue(true);
						 * 
						 * panelTypeSensor.add(arrayTypeSensor[i]);
						 * 
						 * }
						 */
			...
			public void onClick(ClickEvent event) {
				...
				for (int i = 0; i < SENSORS.size(); i++) {
					if (SENSORS.get(i).getSensor_type().equals(type)) {
						...
						/*
						 * if (chFilter.getValue()) { for (int a = 0; a < arrayFilter.length; a++) {
						 * filter.put(arrayFilter[a].getName(), arrayFilter[a].getValue()); } }
						 */

						/*
						 * entityService.getSensorData(SENSORS.get(i).getId(), dbFrom.getValue(), dbTo.getValue(), new
						 * AsyncCallback<List<MeasureDTO>>() {
						 * 
						 * @Override public void onFailure(Throwable caught) { dbWait.hide();
						 * 
						 * }
						 * 
						 * @Override public void onSuccess(final List<MeasureDTO> result) {
						 * 
						 * if (result.size() != 0) { group.put(name, result); if (type.toUpperCase().equals("PHOTO")) {
						 * int numRows = (int) result.size() / 5; int numColumns = 5; Grid grid = new Grid(numRows + 1,
						 * numColumns); grid.setCellPadding(3); int i = 0;
						 * 
						 * for (int row = 0; row <= numRows; row++) { for (int col = 0; col < numColumns; col++) {
						 * 
						 * if (i < result.size()) { byte[] image2 = result.get(i).getImage();
						 * 
						 * char[] encode = Base64Coder.encode(image2); String x = new String(encode);
						 * 
						 ...
						 * ;
						 * 
						 * image.setUrl("data:image/png;base64," + x); image.setSize("25%", "25%");
						 * 
						 * Image imageFull = new Image(); imageFull.setUrl("data:image/png;base64," + x);
						 * 
						 * final DialogBox imagePopup2 = new DialogBox(); imagePopup2.setAnimationEnabled(true);
						 * imagePopup2.setGlassEnabled(true); imagePopup2.setText(result.get(i).getMeasure_date() + "");
						 * imagePopup2.setWidget(imageFull);
						 * 
						 * imageFull.addClickHandler(new ClickHandler() { public void onClick(ClickEvent event) {
						 * imagePopup2.hide(); } });
						 * 
						 * image.addClickHandler(new ClickHandler() { public void onClick(ClickEvent event) {
						 * imagePopup2.center(); imagePopup2.show(); } }); grid.setWidget(row, col, image); } else {
						 * break; } i++; } }
						 * 
						 * DecoratorPanel decPanel = new DecoratorPanel(); decPanel.add(grid);
						 * formPictures.add(decPanel); formPanel.add(formPictures);
						 * formPanel.setCellHorizontalAlignment(formPictures, HasHorizontalAlignment.ALIGN_CENTER);
						 * 
						 * } } else { group.put(name, null); }
						 * 
						 * // last result has arrived, process data if (group.size() == sf) { String data =
						 * GraphUtils.generateStringData(group); if (data == null) {
						 * 
						 * final DialogBox noDatas = new DialogBox(); noDatas.setAnimationEnabled(true);
						 * noDatas.setGlassEnabled(true); noDatas.setText("There is no data for display."); Button close
						 * = new Button("Close"); close.addClickHandler(new ClickHandler() { public void
						 * onClick(ClickEvent event) { noDatas.hide(); } }); noDatas.setWidget(close); noDatas.show();
						 * noDatas.center(); } else { GraphUtils.generateNVD3(measure_unit, "", data); } }
						 * 
						 * dbWait.hide(); } });
						 */
					}
